// Auto-Append feature 
let _base, _stack = [];    // Stack for storing _base positions

// set a new basepoint for append, save previous on baseStack
function begin(ID) {
  _stack.push(_base);// Save old base
  //if (_stack.length > 100) throw new Error("");
...
function end(cnt = 1) {
  if (cnt > 1) end(cnt - 1)
  return (_base = _stack.pop())    // restore old stack
}
